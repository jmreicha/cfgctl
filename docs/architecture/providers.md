...
- `Replace`: Discard manually-added entries and write only cfgctl-managed content (supported by AWS, Steampipe, SSH, and Kubernetes non-merge path; no-op under `--kube-merge`/`--kube-merge-only`)
