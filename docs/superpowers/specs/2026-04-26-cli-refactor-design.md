# CLI Refactor Design: Cobra + Viper + Lipgloss
...
move `--ssh-config-path` off the root command, replace raw ANSI color helpers with Lipgloss, add
provider name validation, and confirm steampipe flags are in scope. Cobra is already in use — this
is not a framework swap.

## Approach

Option B: restructure `root.go` minimally, then layer changes. The file is ~320 lines and not
expected to grow significantly — no file splitting needed. The one new file is `styles.go` for
Lipgloss, which is a genuinely separate concern reused across command files.
